...
r-squared is close to one so there is a strong correlation
...
Ones above the line are more expresed as clones and the ones below are more expressed as the ancestor genes. 
The clone has lost expression relative to the ancestor in a lot of genes. 
...
2 outlier genes were completley knocked out and have no correlation with the other gene (ancestor/clone)
...
all values that are smaller than .02 are kept
# Then replot
...
error 100 means data are too tiny
...
multiply to make values bigger and keep straight line
# We'll retry:
...
#Use AIC comparisons: tells you how well you have fit your data with relationship
best fit will be the lowest which is gamma in the data.
...
#tail goes out furthur on the x-axis. Similar mostly to ancestor
...
There are more dominant genes because of the shorter tail and more evenness because the data is more clustered.
